...
class FirstNN(object):
    ...
    def __init__(self, input_dims, num_nodes_lr1, num_classes, param_init='small_std', std=1e-4):
        ...
        if param_init == 'small_std':
             self.params['W1'] = np.sqrt(1/1000) * np.random.randn(input_dims, num_nodes_lr1)
             self.params['b1'] = np.zeros(num_nodes_lr1)
             self.params['W2'] = np.sqrt(1/1000) * np.random.randn(num_nodes_lr1, num_classes)
             self.params['b2'] = np.zeros(num_classes)
            
        elif param_init == 'ninn_std':
             self.params['W1'] = np.sqrt(1/(input_dims)) * np.random.randn(input_dims, num_nodes_lr1)
             self.params['b1'] = np.zeros(num_nodes_lr1)
             self.params['W2'] = np.sqrt(1/(num_nodes_lr1)) * np.random.randn(num_nodes_lr1, num_classes)
             self.params['b2'] = np.zeros(num_classes)
            
        elif param_init == 'Xavier':
             self.params['W1'] = np.sqrt(2/(input_dims)+(num_nodes_lr1)) * np.random.randn(input_dims, num_nodes_lr1)
             self.params['b1'] = np.zeros(num_nodes_lr1)
             self.params['W2'] = np.sqrt(2/(num_nodes_lr1)+(num_classes))* np.random.randn(num_nodes_lr1, num_classes)
             self.params['b2'] = np.zeros(num_classes)
    ...
    def forword(self, X):
        ...
        _z1 = np.dot(X,W1) + b1
        self.f1 = sigmoid_func(_z1)

        W2 = self.params['W2'] 
        b2 = self.params['b2']

        _z2 = np.dot(self.f1, W2) + b2
        prob_scores = np.exp(_z2)/np.sum(np.exp(_z2), axis=1 , keepdims= True)
    ...
    def loss(self, Y, prob_scores):
        ...
        #############################################################################
        N = Y.shape[0]
        C_log = -np.log(prob_scores[range(N), Y])
        loss = Y.T @ C_log
    ...
    def backword(self, X, Y, prob_scores):
        ...
        #############################################################################
        N, D = X.shape
        prob_scores_neg_y = copy.deepcopy((prob_scores))
        prob_scores_neg_y[(range(N),Y)] -= 1                   #NxC
        
        grads['W2'] = np.dot(self.f1.T, prob_scores_neg_y)/N         #MxC
        grads['b2'] = np.sum(prob_scores, axis=0)/N               #C

        d_z1 = np.dot(prob_scores_neg_y, self.params['W2'].T)*(self.f1*(1-self.f1)) #NxN
        
        grads['W1'] = np.dot(X.T, d_z1)/N 
        grads['b1'] = np.sum(d_z1, axis=0)/N
    ...
    def optimizer(self, grads, update_rule='gd'):
        ...
        if self.update_rule == 'gd':
            ...
            for param in self.params:
                 self.velocity[param] = self.beta_moment*self.velocity[param] - self.learning_rate*grads[param]
                 self.params[param] += self.velocity[param]
                  
        elif self.update_rule == 'nm_gd':
             for param in self.params:
                p_velocity = self.velocity[param]
                self.velocity[param] = self.beta_moment * self.velocity[param] - self.learning_rate*grads[param]
                self.params[param] += -self.beta_moment * p_velocity + (1 + self.beta_moment) * self.velocity[param]
    ...
    def predict(self, X, best_param=False):
        ...
        if best_param == True:
            ...
            _z1 = X.dot(self.best_params['W1']) + self.best_params['b1']
            f1 = sigmoid_func(_z1)
            _z2 = f1.dot(self.best_params['W2']) + self.best_params['b2']
            
            Y_pred = np.argmax(_z2, axis=1) 
        else:
            _z1 = X.dot(self.params['W1']) + self.params['b1']
            f1 = sigmoid_func(_z1)
            _z2 = np.dot(f1, self.params['W2']) + self.params['b2']
